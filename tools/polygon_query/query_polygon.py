...
# Add repo root to path so `from shared.db import ...` works regardless of cwd.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))
from shared.db import get_engine, safe_read_sql  # noqa: E402
...
def main():
    ...
    # Connect — uses shared/db.py, which sanitizes errors so no host/user/password
    # ever reaches stdout/stderr (even on connection failure).
    ...
    for i, (label, ewkt) in enumerate(polygons):
        ...
        df = safe_read_sql(QUERY, engine, params={"polygon_ewkt": ewkt})
